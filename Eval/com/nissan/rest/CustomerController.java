...
public class CustomerController {
	...
	public ResponseEntity<APIResponse> Deposit(@PathVariable int AccountNo, @PathVariable int DepositAmt,
		...
		if(isAccountNo(toString(AccountNo)))
		{
	...
	@PutMapping("/customers/{AccountNo1}&{Amount}")
	public ResponseEntity<APIResponse> Transfer(@PathVariable int AccountNo1,@PathVariable int Amount,
			@RequestHeader(value = "authorization", defaultValue = "") String auth,@RequestBody Customer cust) throws AccessDeniedException {// get
		jwtutil.verifyAdmin(auth);
		
		if(isAccountNo(toString(AccountNo1)))
		{if (cust.getAccountNo() != null) {
			
			custservice.Transfer(AccountNo1, Amount);
			apiresponse.setData("Valid Account");
			apiresponse.setStatus(200);
			return ResponseEntity.status(apiresponse.getStatus()).body(apiresponse);
		...
		apiresponse.setData("Invalid Account");
		apiresponse.setStatus(200);
		return ResponseEntity.status(apiresponse.getStatus()).body(apiresponse);
